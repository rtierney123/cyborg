...
public class projectileScriptforEnemy : MonoBehaviour {
    ...
    {
        print("fire");
        facingDirection = gameObject.GetComponent<AIPath>().direction;
        ...
        moveDirection = gameObject.GetComponent<AIPath>().projectileDir;
        //GameObject bullet = Instantiate(projectile, bulletplace, Quaternion.identity);
        //bullet.transform.parent = this.gameObject.transform;
        Instantiate(projectile, bulletplace, Quaternion.identity).transform.parent = this.gameObject.transform;
    }
